...
export default class LoginPage {
  get user() {
    return $(
      'android=new UiSelector().packageName("com.example.lojinha").className("android.widget.EditText").instance(0)',
    );
  }
  get password() {
  ...
  async loginLojinha(usuario, senha) {
    await Promise.all([this.fillUser(usuario), this.fillPassword(senha)]);
